...
class AppLib {
    ...
    static function pData(){
        ...
        $purchase = self::remote_get_contents(UPDATE_URL.'verify.php?code='.$pc);
    ...
    static function _chart_received_amount($year,$month){
        $total = 0;
        $query = "SELECT * FROM fx_payments WHERE MONTH(payment_date) = '$month' AND YEAR(payment_date) = '$year'";
        $payments = self::$db->query($query)->result();
        // $payments = self::$db->where(array('month_paid'=>$month,'year_paid' => $year))->get('payments')->result();
    ...
    static function date_formatter($str){
        return date_format(date_create_from_format(config_item('date_php_format'), $str), 'Y-m-d');
    }
    ...
    function get_updates($remote = FALSE) {
        ...
        $result = $this->ci->db->query("SHOW TABLES LIKE 'fx_updates'")->result();
