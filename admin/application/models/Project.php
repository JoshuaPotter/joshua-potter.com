...
class Project extends CI_Model
{
    ...
    static function by_id($project)
    {
        return self::$db->where('project_id',$project)->get('projects')->row();
    }
    ...
    {
        return self::$db->where('status !=','Done')->get('projects',$limit)->result();
    }
    ...
    {
        return self::$db->get('tasks')->result();
    }
    ...
    static function get_progress($project = NULL)
    {
        ...
        if($auto_progress == 'TRUE'){
            ...
            return round(self::$db->select_avg('task_progress')->where('project',$project)->get('tasks')->row()->task_progress,2);
        }else{
    ...
    static function get_hours($project,$billable = NULL,$type = 'projects'){

        $billable = ($billable == NULL) ? '1' : '0';

        $sql = "SELECT start_time,end_time,project,end_time - start_time time_spent 
				FROM fx_project_timer WHERE project = '$project' AND billable='$billable'";

        if($type == 'tasks'){

            $sql = "SELECT start_time,end_time,pro_id,end_time - start_time time_spent 
				FROM fx_tasks_timer WHERE pro_id = '$project' AND billable='$billable'";
        }
    ...
    static function time_by_user($user,$billable = '1',$project){
        $data = array();
        $sql1 = "SELECT start_time,end_time,end_time - start_time time_spent 
						  FROM fx_project_timer 
						  WHERE user = '$user' AND billable='$billable' AND project='$project'";
        $p_time = self::$db->query($sql1)->result();
        $pt = array();
        foreach ($p_time as $key => $p) { $pt[] = $p->time_spent; }
        $data['projects_time'] = array_sum($pt);

        $sql2 = "SELECT start_time,end_time,end_time - start_time time_spent 
					   FROM fx_tasks_timer 
					   WHERE user = '$user' AND billable='$billable' AND pro_id='$project'";

        $t_time = self::$db->query($sql2)->result();
        $tt = array();
        foreach ($t_time as $key => $t) { $tt[] = $t->time_spent; }
        $data['tasks_time'] = array_sum($tt);
        return (object)$data;
    }
    ...
    static function task_timer($task){
        $sql = "SELECT start_time,end_time,pro_id,end_time - start_time time_spent
					FROM fx_tasks_timer WHERE task = '$task' AND billable = '1'";
        $res = self::$db->query($sql)->result();
        $a = array();
        foreach ($res as $key => $t) {
            $a[] = $t->time_spent;
    ...
    static function milestone_progress($id){
        ...
        return ($tasks > 0) ? round(($row->task_progress/$tasks),1) : 0;
    ...
    static function activity($id){
        ...
        return self::$db->where(array('module'=>'projects','module_field_id'=>$id))->get('activities')->result();
    }
    ...
    static function convert_estimate_to_project($estimate){
        ...
        $data = array(
                        ...
                        'fixed_price'   => Estimate::due($estimate),
                        'progress'      => 0,
                        'auto_progress' => 'TRUE',
    ...
    static function delete($project){
        ...
        foreach (self::has_bugs($project) as $bug) {
            foreach (self::bug_has_files($bug->bug_id) as $key => $f) {
                $fullpath = './resource/bug-files/'.$f->file_name;
        ...
        foreach (self::has_tasks($project) as $task) {
            foreach (self::task_has_files($task->t_id) as $key => $f) {
                $fullpath = './resource/project-files/'.$f->path.$f->file_name;

                if($f->path == NULL) $fullpath = './resource/project-files/'.$f->file_name;
