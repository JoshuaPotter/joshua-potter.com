...
class Updates extends MX_Controller {
    ...
    function plugins() {
        ...
        $data['plugins'] = $this->db->order_by('title','asc')->get('plugins')->result();
        $this->template
                ->set_layout('users')
                ->build('plugins', isset($data) ? $data : NULL);
    }
    ...
    {
        $plug = $this->db->where('id', $id)->get('plugins')->row();
        
        $path = "./resource/updates/".$id."_".$plug->version.".zip";
        $url = PLUGINS_URL.$plugin."/".$id."_".$plug->version.".zip";
        ...
        redirect('updates/plugins');
    }
    
    function scan_plugins() {
        $this->load->model(array('plugin'));
        $path = './application/modules/';
        $module = array();
        $files = scandir($path);
        foreach ($files as $file) {
            if (\is_dir($path.$file) && \file_exists($path.$file."/.foplugin")) {
                $info = modules::run($file."/info");
                if ($this->db->where('id', $info["id"])->get('plugins')->num_rows() == 0 ) {
                    $this->plugin->register_plugin($info);
                }
            }
        }
    }
    ...
    function backup() {
        ...
        $ignore = array("./resource/backup","./resource/updates");
    ...
    function install() {
        $releases = json_decode(Applib::remote_get_contents(UPDATE_URL . 'version.php'), true);
    ...
    function migrate_db($version = NULL) {
        ...
        //$this->db_sync();
        $url = UPDATE_URL.'folite/db/upgrade_'.$version.'.sql';
    ...
    function db_json($settings = array()) {
        ...
        $data = defined(JSON_UNESCAPED_UNICODE) ? json_encode($db, JSON_UNESCAPED_UNICODE) : json_encode($db);
        write_file($url, $data);
    }
    ...
    function db_sync() {
        $this->load->database();
        $this->load->helper('file');
        $source = Applib::remote_get_contents(UPDATE_URL."folite/db/db.json");
...
//        $this->db_upd_tables($src['data']);
    ...
    function db_check() {
        $this->load->database();
        $this->load->helper('file');
        $src = Applib::remote_get_contents(UPDATE_URL."folite/db/db.json");
    ...
    public function getFile($file_name){
        ...
        $fullpath = './resource/backup/'.$file_name;
    ...
    public function delFile($file_name){
        ...
        if($fullpath){
            ...
            App::go_to($_SERVER['HTTP_REFERER'],'error',lang('operation_failed'));
        }
    }
