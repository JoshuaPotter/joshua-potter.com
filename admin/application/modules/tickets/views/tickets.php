          ...
          <section class="panel panel-default">
            ...
            <?=lang('all_tickets')?>
                ...
                <table id="table-tickets<?=($archive) ? '-archive':''?>" class="table table-striped b-t b-light AppendDataTables">
                  <thead>
                   ...
                   <th class="col-lg-1"><?=lang('reporter')?></th>
                    <?php } ?>
                    <th class="col-date col-lg-2"><?=lang('date')?></th>                    
                    <th class="col-options no-sort col-lg-1"><?=lang('priority')?></th>
              ...
              <?php $rep = $this->db->where('ticketid',$t->id)->get('ticketreplies')->num_rows();
                ...
                <a class="text-info small <?=($t->status == 'closed') ? 'text-lt' : ''; ?>" href="<?=base_url()?>tickets/view/<?=$t->id?>" data-toggle="tooltip" data-title="<?=lang('ticket_not_replied')?>">
                     <i class="fa fa-circle-o text-danger"></i>
                     <?php }else{ ?>
                <a class="text-info small <?=($t->status == 'closed') ? 'text-lt' : ''; ?>" href="<?=base_url()?>tickets/view/<?=$t->id?>">
                      <?php } ?>

                     <?=word_limiter($t->subject, 5);?>
                     </a><br>
                     <?php if($rep == 0){ ?>
                     <span class="small text-danger">Pending for <?=Applib::time_elapsed_string(strtotime($t->created));?></span>
                     <?php } ?>
                      ...
                      <?php
                      if($t->reporter != NULL){ ?>
                        <a class="pull-left small thumb-sm avatar" data-toggle="tooltip" title="<?php echo User::login_info($t->reporter)->email; ?>" data-placement="right">
                      ...
                      <?php } ?>
                       ...
                       <td class="small">
                       <?=strftime(config_item('date_format').' %H:%M', strtotime($t->created));?><br/>
                      <span class="text-primary"><?=Applib::time_elapsed_string(strtotime($t->created));?></span>
                       </td>

                      <td>
                      <span class="label label-<?php if($t->priority == 'Urgent') { echo 'danger'; }elseif($t->priority == 'High') { echo 'warning'; }else{ echo 'default'; } ?> small"> <?=$t->priority?></span>
                      </td>
