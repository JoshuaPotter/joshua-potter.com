<section id="content">
	...
	<img src="<?php echo User::avatar_url($user->id); ?>" class="img-circle">
														
	<span class="label label-<?=($user->banned == '1') ? 'danger': 'success'?>"><?=$user->username?></span>

	<?php if($user->role_id == '3') { ?>
	 <strong class="small"><?=config_item('default_currency_symbol')?><?=User::profile_info($user->id)->hourly_rate;?>/<?=lang('hour')?></strong>
	 <?php }?>
				</a>
				</td>

				<td class="small"><?=$info->fullname?></td>
				<td class="small">
