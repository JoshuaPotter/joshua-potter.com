<section id="content"> <section class="vbox">
        <section class="scrollable padder">
            ...
            if ($user_company > 0) {
                ...
                ?>
                <?php $cur = Client::client_currency($user_company); ?>
            ...
            <div class="m-b-md">
                ...
                <?php if($client_outstanding > 0){ ?>
                    ...
                    <div class="alert alert-info hidden-print">
                        ...
                        <?=lang('your_balance_due')?> <?=Applib::format_currency($cur->code, $client_outstanding)?></strong>
                    </div>
                <?php } ?>
            ...
            <section class="panel panel-default">
				...
				<span class="fa-stack fa-2x pull-left m-r-sm"> <i class="fa fa-circle fa-stack-2x text-danger"></i> <i class="fa fa-coffee fa-stack-1x text-white"></i>
					...
					<strong><?=App::counter('projects',array('client'=>$user_company)); ?> </strong>
				...
				<span class="fa-stack fa-2x pull-left m-r-sm"> <i class="fa fa-circle fa-stack-2x text-info"></i> <i class="fa fa-envelope fa-stack-1x text-white"></i>
					...
					<span class="h3 block m-t-xs">
					<strong><?=App::counter('messages',array('user_to'=>$user,'deleted'=>'No'))?> </strong>
				</span> <small class="text-muted text-uc"><?=lang('messages')?>  </small> </a>
                    </div>


                    <?php
                    $invoices = Invoice::get_client_invoices($user_company);
                    $invoicestatus = 0;
                    foreach($invoices as $key => $inv){
                        if (Invoice::payment_status($inv->inv_id) != 'fully_paid'){ $invoicestatus += 1; }
                    }
                    ?>
                    <div class="col-sm-6 col-md-3 padder-v b-r b-light">
...
<span class="fa-stack fa-2x pull-left m-r-sm"> <i class="fa fa-circle fa-stack-2x text-warning"></i> <i class="fa fa-suitcase fa-stack-1x text-white"></i>
				...
				<span class="fa-stack fa-2x pull-left m-r-sm"> <i class="fa fa-circle fa-stack-2x text-success"></i> <i class="fa fa-calendar-o fa-stack-1x text-white"></i>
					...
					<span class="h3 block m-t-xs">
					<strong><?=App::counter('activities',array('user'=>$user))?> </strong>
                    ...
                    <section class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table class="table table-striped m-b-none text-sm">
                                <thead>
                                <tr>
                                    <th class="col-md-4"><?=lang('project_name')?> </th>
                                    <th class="hidden-xs col-md-4"><?=lang('progress')?></th>
                                    <th class="col-currency col-md-2"><?=lang('project_cost')?></th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a class="btn  btn-success btn-xs" href="<?=base_url()?>projects/view/<?=$project->project_id?>">
                                                <i class="fa fa-suitcase text"></i> <?=lang('project')?>
                                            </a>
						...
						<?php echo App::counter('projects',array('client'=>$user_company,'progress >='=>'100')); ?>
							...
							<?php echo App::counter('messages',array('user_to'=>$user,'status'=>'Unread'))?>
                                ...
                                <?php
                                $ticketnumber = App::counter('tickets',array('reporter'=>$user, 'status'=>'open')) + App::counter('tickets',array('reporter'=>$user, 'status'=>'in progress'));
                                ?>
