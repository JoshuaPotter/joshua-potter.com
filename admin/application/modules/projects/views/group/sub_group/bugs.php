...
<header class="header bg-white b-b clearfix">
    ...
    <div class="table-responsive">
      ...
      <?php foreach (Project::has_bugs($project_id) as $key => $b) { 
               ...
               <a class="pull-left thumb-xs avatar" data-toggle="tooltip" data-original-title="<?=User::displayName($b->reporter)?>">

        
      <img src="<?=User::avatar_url($b->reporter); ?>" class="img-circle">
      
          </a>



                        <a class="text-info m-l-xs" href="<?=base_url()?>projects/view/<?=$b->project?>?group=bugs&view=bug&id=<?=$b->bug_id?>" data-original-title="<?=$b->bug_description?>" data-toggle="tooltip" data-placement="right" title = "" ><?=word_limiter($issue_title,4)?></a>
