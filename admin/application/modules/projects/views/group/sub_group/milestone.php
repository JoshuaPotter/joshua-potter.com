      ...
      <div class="row m-t-sm">
        <div class="col-sm-12 m-b-xs">
          ...
          <?php  if(User::is_admin() || Project::is_assigned(User::get_id(),$project_id)){ ?>
           <a href="<?=base_url()?>projects/milestones/add_task/<?=$m->id?>/<?=$project_id?>" data-toggle="ajaxModal" class="btn btn-sm btn-<?=config_item('theme_color')?>"><?=lang('add_task')?></a>

            <a href="<?=base_url()?>projects/milestones/edit/<?=$m->id?>" data-toggle="ajaxModal" class="btn btn-sm btn-<?=config_item('theme_color')?>"><?=lang('edit_milestone')?></a>
        ...
        <tbody>
          <?php foreach (Project::milestone_has_tasks($m->id) as $key => $t) {
            ...
            <tr>
              ...
              <td>
                <div class="inline ">
                  <div class="easypiechart text-<?=config_item('theme_color')?>" data-percent="<?=$t->task_progress?>" data-line-width="5" data-track-Color="#f0f0f0" data-bar-color="#<?php echo ($t->task_progress == 100) ? '8ec165' : 'fb6b5b'; ?>" data-rotate="270" data-scale-Color="false" data-size="50" data-animate="2000">
