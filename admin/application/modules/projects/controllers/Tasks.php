...
class Tasks extends MX_Controller {
    ...
    function add() {
        if ($this->input->post()) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if(User::is_client()) { $assign = unserialize($assign); }
                if (count($assign) > 0) {
        ...
        if ($this->input->post()) {
            Applib::is_demo();
                ...
                if ($action == 'edit') {
                    ...
                    $fullpath = './resource/project-files/'.$file_data->path.$file_data->file_name;
            ...
            if ($action == 'edit') {
                ...
                $data['task'] = $file->task;
                $fullpath = './resource/project-files/'.$file->path.$file->file_name;
    ...
    function tracking() {
        ...
        if ($action == 'Off') {
            ...
            $task_start = Project::view_task($task)->start_time;
            ...
            $data = array(
                'timer_status' => $action,
                'timer_started_by' => User::get_id(),
                'start_time' => time()
                );
            App::update('tasks',array('t_id' => $task),$data);
            $message = 'timer_started_success';
        }
        Applib::go_to('projects/view/'.$project.'?group=tasks', 'success', lang($message));
    }

    function _timer_started_by($task) {
        $started_by = Project::view_task($task)->timer_started_by;
    ...
    function download() {
        ...
        if($f->path == NULL){
            $fullpath = './resource/project-files/'.$f->file_name;
        }
