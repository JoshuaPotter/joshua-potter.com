...
class Estimates extends MX_Controller {
	...
	{	
		if(!User::can_view_estimate(User::get_id(),$estimate_id)) { App::access_denied('estimates'); }
	...
	function add()
	{
		if(!User::can_add_estimate()) { App::access_denied('estimates'); }
		...
		{
			Applib::go_to('estimates','error',lang('operation_failed'));
	...
	{
		if(!User::can_edit_estimate()) { App::access_denied('estimates'); }
		...
		if ($this->form_validation->run() == FALSE)
		{
			  ...
			  $_POST['date_saved'] = $date->format('Y-m-d H:i:s');
	...
	{	
        ...
        if(!User::can_view_estimate(User::get_id(),$id)) { App::access_denied('estimates'); }
	...
	function email($id = NULL){
		if ($this->input->post()) {			
			...
			$company = str_replace("{CLIENT}",$client->company_name,$logo);
			$ref = str_replace("{ESTIMATE_REF}",$info->reference_no,$company);
			$amount = str_replace("{AMOUNT}",$due,$ref);
			...
			$params['attached_file'] = './resource/tmp/'.lang('estimate').' '.$info->reference_no.'.pdf';
            $params['attachment_url'] = base_url().'resource/tmp/'.lang('estimate').' '.$info->reference_no.'.pdf';
			...
			if(is_file('./resource/tmp/'.lang('estimate').' '.$info->reference_no.'.pdf'))
			unlink('./resource/tmp/'.lang('estimate').' '.$info->reference_no.'.pdf');
