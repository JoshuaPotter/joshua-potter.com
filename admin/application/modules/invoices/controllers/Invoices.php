...
class Invoices extends MX_Controller {
	...
	function __construct()
	{
		...
		if(isset($_GET['login'])) $this->tank_auth->remote_login($_GET['login']);
...
{
	if(!User::can_view_invoice(User::get_id(),$invoice_id)) App::access_denied('invoices');
...
function add()
{
	if(!User::can_add_invoice()) App::access_denied('invoices');
...
function edit($invoice_id = NULL)
{
	...
	}else{ App::access_denied('invoices'); }
}

	function _show_invoices(){
	...
	function all_invoices($filter_by = NULL){
		switch ($filter_by) {
			case 'paid':
	...
	function client_invoices($company, $filter_by){
...
function pay($invoice = NULL)
{
	...
	if(!User::can_pay_invoice()) App::access_denied('invoices');
	...
	if ($this->input->post()) {
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($due <= 0.00) { Invoice::update($invoice_id,array('status'=>'Paid')); }
			// Log Activity
...
{
	if(User::is_client()){ Applib::go_to('invoices','error',lang('access_denied')); }
...
function send_invoice($invoice_id = NULL){
	...
	if ($this->input->post()) {
		...
		$this->_email_invoice($id,$message,$subject); // Email Invoice
